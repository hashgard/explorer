    ...
    <div class="address-detail-content" v-if="info">
      <card title="Address Information">
        <data-item label="Balance">
          <data-amount v-if="!isEmpty(info)" :list="info" />
          <span v-else>-</span>
        </data-item>
      ...
      <card title="delegations">
        <data-item label="Total Balance" v-if="!isEmpty(addressDelegators)">
          <data-amount :list="[{ denom: 'ugard', amount: addressDelegationsBalance }]" />
        </data-item>
        <data-item label="Total Shares" v-if="!isEmpty(addressDelegators)">
          <data-amount :list="[{ denom: 'ugard', amount: addressDelegationsShares }]" />
        </data-item>
        <el-table class="table" :data="addressDelegators" style="width: 100%" :empty-text="$t('global.noneData')">
          ...
          <el-table-column label="Validator Address">
            <template slot-scope="scope">
              <hg-link type="validator" :ellipsis="false" :content="scope.row.validator_address"></hg-link>
            </template>
          </el-table-column>
        </el-table>
      </card>
      <card title="redelegations">
        <el-table class="table" :data="addressRedelegations" style="width: 100%" :empty-text="$t('global.noneData')">
          <el-table-column label="Amount">
            <template slot-scope="scope">
          ...
          <el-table-column label="Creation Height">
            <template slot-scope="scope">
              <hg-link type="block" :ellipsis="false" :content="scope.row.entries.creation_height.toString()"></hg-link>
            </template>
          </el-table-column>
          <el-table-column label="Validator From">
            <template slot-scope="scope">
              <hg-link type="validator" :ellipsis="false" :content="scope.row.validator_src_address"></hg-link>
            </template>
          </el-table-column>
          <el-table-column label="Validator To">
            <template slot-scope="scope">
              <hg-link type="validator" :ellipsis="false" :content="scope.row.validator_dst_address"></hg-link>
            </template>
          </el-table-column>
        </el-table>
      </card>
      <card title="unbonding delegations">
        <el-table class="table" :data="addressUnbonding" style="width: 100%" :empty-text="$t('global.noneData')">
          <el-table-column label="Amount">
            <template slot-scope="scope">
          ...
          <el-table-column label="Creation Height">
            <template slot-scope="scope">
              <hg-link type="block" :ellipsis="false" :content="scope.row.entries[0].creation_height.toString()"></hg-link>
            </template>
          </el-table-column>
          <el-table-column label="Validator Address">
            <template slot-scope="scope">
      ...
      <card title="Delegator Rewards">
        <data-item label="Total Rewards" v-if="!isEmpty(rewards.total)">
          <data-amount :list="rewards.total" />
        </data-item>
        <el-table v-if="!isEmpty(rewards)" :data="rewards.rewards" style="width: 100%" :empty-text="$t('global.noneData')">
          <el-table-column label="Rewards">
            <template slot-scope="scope">
              <data-amount v-if="scope.row.reward" :list="scope.row.reward" />
              <span v-else>-</span>
            </template>
          </el-table-column>
          <el-table-column label="Validator Address">
...
export default {
  ...
  async mounted() {
    ...
    this.$store.dispatch('transactions/fetchAddressRedelegations', this.address);
    ...
    this.withdrawAddress = await this.$store.dispatch('validators/fetchWithdrawAddress', this.address);
    this.$store.dispatch('transactions/fetchAddressTxList', {
      address: this.address,
      id: ''
    });
